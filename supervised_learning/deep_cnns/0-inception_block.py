...
def inception_block(A_prev, filters):
    ...
    conv_3x3_reduce = K.layers.Conv2D(filters=F3R, kernel_size=(1, 1),
                             padding='same', activation='relu')(A_prev)
    conv_3x3 = K.layers.Conv2D(filters=F3, kernel_size=(3, 3), padding='same',
    ...
    conv_5x5_reduce = K.layers.Conv2D(filters=F5R, kernel_size=(1, 1),
                             padding='same', activation='relu')(A_prev)
    conv_5x5 = K.layers.Conv2D(filters=F5, kernel_size=(5, 5), padding='same',
    ...
    output = K.layers.concatenate([conv_1x1, conv_3x3, conv_5x5, conv_pool_proj],
                         axis=-1)
