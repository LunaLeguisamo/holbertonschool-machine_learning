...
def get_first_launch():
    ...
    # Get rocket name
    rocket_resp = requests.get(f"https://api.spacexdata.com/v4/rockets/{rocket_id}")
    rocket_name = rocket_resp.json().get("name")

    # Get launchpad info
    launchpad_resp = requests.get(f"https://api.spacexdata.com/v4/launchpads/{launchpad_id}")
    launchpad_data = launchpad_resp.json()
    launchpad_name = launchpad_data.get("name")
    launchpad_locality = launchpad_data.get("locality")

    return f"{name} ({date_local}) {rocket_name} - {launchpad_name} ({launchpad_locality})"
